...
- **效果：** `appear`/`disappear`（出现/消失）、`fade-in`/`fade-out`（淡入/淡出）、`fly-in`/`fly-out`（四方向飞入/飞出）、`wipe-in`（擦除）、`zoom-in`/`zoom-out`（缩放入/出）、`spin`（旋转）、`grow`/`shrink`（放大/缩小），以及 `path` —— 由直线段和三次贝塞尔曲线组成的自定义路径动画。
- **时序：** `data-anim-trigger="click|with|after"` 与 PowerPoint 的「单击时 / 与上一动画同时 / 上一动画之后」一一对应（默认 `after`，所以只写一个 `data-anim="fade-in"` 就会在翻到该页时自动播放）；`data-anim-delay`、`data-anim-duration`、`data-anim-order` 用来微调时间线。
